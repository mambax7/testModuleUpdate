 ...
 * @return string
 */
function testmoduleupdate_block_addCatSelect($cats)
{
    $cat_sql = '(';
    if (\is_array($cats)) {
 ...
 */
function testmoduleupdateGetMyItemIds($permtype, $dirname)
{
    global $xoopsUser;
    static $permissions = [];
    ...
    $testmoduleupdateModule = $moduleHandler->getByDirname($dirname);
    $groups = \is_object($xoopsUser) ? $xoopsUser->getGroups() : \XOOPS_GROUP_ANONYMOUS;
    $grouppermHandler = \xoops_getHandler('groupperm');
    $itemIds = $grouppermHandler->getItemIds($permtype, $groups, $testmoduleupdateModule->getVar('mid'));
    return $itemIds;
}
 ...
 * @return int
 */
function testmoduleupdateNumbersOfEntries($mytree, $testfields, $entries, $cid)
{
    $count = 0;
...
function testmoduleupdateMetaKeywords($content)
{
...
function testmoduleupdateMetaDescription($content)
{
    global $xoopsTpl, $xoTheme;
    $myts = MyTextSanitizer::getInstance();
    $content = $myts->undoHtmlSpecialChars($myts->displayTarea($content));
 ...
 */
function testmoduleupdate_RewriteUrl($module, $array, $type = 'content')
{
    $comment = '';
    ...
    if (isset($array['topic_alias']) && $array['topic_alias']) {
        ...
        $topic_name = testmoduleupdate_Filter(xoops_getModuleOption('static_name', $module));
    }
 ...
 * @return string $url
 */
function testmoduleupdate_Filter($url, $type = '') {
