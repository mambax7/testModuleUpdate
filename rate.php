...
switch ($op) {
    ...
    case 'save':
        ...
        if (!$rate_allowed) {
            \redirect_header('index.php', 3, \_MA_TESTMODULEUPDATE_RATING_NOPERM);
        }
        ...
        switch ((int)$helper->getConfig('ratingbars')) {
            ...
            default:
                \redirect_header('index.php', 3, \_MA_TESTMODULEUPDATE_RATING_VOTE_BAD);
                exit;
                break;
        ...
        if ($ratingsHandler->insert($ratingsObj)) {
            ...
            if (Constants::TABLE_ARTICLES === $source) {
                ...
                if ($articlesHandler->insert($articlesObj)) {
                    \redirect_header($redir, 2, \_MA_TESTMODULEUPDATE_RATING_VOTE_THANKS);
                } else {
                    \redirect_header('articles.php', 3, \_MA_TESTMODULEUPDATE_RATING_ERROR1);
                }
                unset($articlesObj);
            }
            if (Constants::TABLE_TESTFIELDS === $source) {
                ...
                if ($testfieldsHandler->insert($testfieldsObj)) {
                    ...
                    \redirect_header('testfields.php', 3, \_MA_TESTMODULEUPDATE_RATING_ERROR1);
                }
                unset($testfieldsObj);
            }

            \redirect_header('index.php', 2, \_MA_TESTMODULEUPDATE_RATING_VOTE_THANKS);
        }
        // Get Error
